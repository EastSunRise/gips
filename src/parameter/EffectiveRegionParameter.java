...
public class EffectiveRegionParameter {
                ...
                try {
                        ...
                        for (int i = 0; i < lh.linesplit.length; i++) {
                                ...
                                if (temp.contains("promoter")) {
                                        ...
                                        common.CExecutor.stopProgram("Don't find region: " + temp);
                                        
                                }
                        }
                } catch (ArrayIndexOutOfBoundsException e) {
                        common.CExecutor.stopProgram("region parameter is not set correctly or empty! ");
                } catch (NumberFormatException e1) {
                        common.CExecutor.stopProgram("Please input an integer, for example Promoter=150,SpliceSite=2");
                }
        ...
        private void setPromoterLength(int length) {
                if (length < 0) {
                        length = 200;
                        common.CExecutor.println("Promoter length is not correct and has been set 200! ");
                }
        ...
        private void setSpliceSite(int length) {
                if (length < 0 || length > 3) {
                        length = 2;
                        common.CExecutor.println("splice site is not correct and has been set 2! ");
                }
                spliceSite = length;
        }
        ...
        public void printRegionParameter() {
                common.CExecutor.println("\t\t   Promoter length: " + this.getPromoterLength());
                common.CExecutor.println("\t\t   Splice site: " + this.getSpliceSite());
                common.CExecutor.println("\t\t   5UTR: " + this.isConsider5UTR());
                common.CExecutor.println("\t\t   3UTR: " + this.isConsider3UTR());
                common.CExecutor.println("\t\t   CDS: " + this.isConsiderCDS());
                common.CExecutor.println("\t\t   Exon: " + this.isConsiderExon());
