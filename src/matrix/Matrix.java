...
public class Matrix {
       ...
       public Matrix(String matrixName) {
              this.matrix= file.Config.getMatrix(matrixName);
...
//              if(originAA.length()!=mutatedAA.length()||originAA.length()!=1||mutatedAA.length()!=1)
//                      return 1;//we do not regard codon insertion or deletion as a big difference
//              if(originAA.equals("-")||mutatedAA.equals("-"))return 1;
              String temp=originAA+mutatedAA;
              if(!this.matrix.keySet().contains(temp)){
                      return 1;//origin * to mutation ? (eg. codon lost) 
       ...
       public static boolean isMatrixName(String matrixName){
               if(file.Config.getMatrix(matrixName)==null){
                       common.CExecutor.println("Don't find "+matrixName+" in config file");
                       return false;
               }else return true;
       }
