...
public class ResultOutputer {
        ...
        public void outputResult(){
                ...
                common.CExecutor.println(common.CExecutor.getRunningTime()+"Result file output: "+Init.getWorkingDirectroy());
        ...
        private void createEmptyResultFile(){
                ...
                String gipsFilePath=workingDirectory+common.CExecutor.getFileSeparator()+Config.getItem("RESULT_GIPS_FILE");
                CommonOutputFile gipsFile=FileFactory.getOutputFile(gipsFilePath);
                if(gipsFile.isFile()){
                        new FileHandler().fileChannelCopy(new File(gipsFilePath),new File(Init.getResultArchiveFolderDirectory()+common.CExecutor.getFileSeparator()+"Archive_"+common.CExecutor.getCurrentTime().replace(":", "-") +"_"+gipsFile.getFileName()));
                }
                gipsFile.write(""
                        + "#================================================================"
                        ...
                        + "\n#"
                        + "\n# Job id: "+job.getMD5()
                        + "\n# Completion time:"+common.CExecutor.getCurrentTime()
        ...
        private void dumpFilterOutput(){
                ...
                for(String sampleName:sampleParameterBag.getSamplesNamesList()){
                        SampleParameter sampleParameter=sampleParameterBag.getSample(sampleName);
                        CommonOutputFile outputFile=FileFactory.getOutputFile(Init.getWorkingDirectroy()+common.CExecutor.getFileSeparator()+sampleName+".filter.gips");
                        outputFile.write("#================================================================"
   ...
   //          System.out.println("\n\n\n"+sb2Zip);
        ...
        private void dumpCandidateGene(CandidateGeneBag candidateGeneBag){
                //output SNP in candidate gene 
                String genesFilePath=Init.getWorkingDirectroy()+common.CExecutor.getFileSeparator()+Config.getItem("CAN_GENE_FILE");
                CommonOutputFile geneFile=FileFactory.getOutputFile(genesFilePath);
                if(geneFile.isFile()){
                        new FileHandler().fileChannelCopy(new File(genesFilePath),new File(Init.getResultArchiveFolderDirectory()+common.CExecutor.getFileSeparator()+"Archive_"+common.CExecutor.getCurrentTime().replace(":", "-")+"_"+geneFile.getFileName()));
                }
                geneFile.write("#================================================================"
                        + "\n#"
                        + "\n# Gene Identification via Phenotype Sequencing - Candidate Gene File\n# Job id: "+job.getMD5()+"   Time:"+common.CExecutor.getCurrentTime()
                        + "\n#================================================================\n\n"
                        + "[Candidate Gene List]\n"
                        + "GENE_ID\tGENE_NAME\tChr\tEFF_REGION_LENGHT\tSIGNIFICANCE\tNUM_VARIANT\tNUM_SAMPLE\n"
                        + ""
